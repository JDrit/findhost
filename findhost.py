...
def matchHosts(hostlist, username, threshold, timeout, filt, password=None):
    ...
    for host in (line for line in (line.split("#")[0].strip() for line in open(hostlist)) if line):
	    ...
	    matchHosts(hostlist, username, threshold, timeout, filt, password)
	    return
	except Exception as e:
	    print "Error: unknown exception cause by trying to connecto to " + host + ", host does not exist"
	    continue
...
def main():
    ...
    parser.add_argument('-o', '--timeout', type=float, default=20.0, help='The default timeout for the ssh connections')
    parser.add_argument('hostlist', type=str, help='The host list to examine')
    args = parser.parse_args()
    matchHosts(args.hostlist, args.user, args.threshold, args.timeout,
